...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        <a class="navbar-brand" href="../Control/controle.php?acao=listar">Ordens de Serviços</a>
        <div class="collapse navbar-collapse" id="navbarSite">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link" href="formCadastroOs.php">Cadastrar</a>
                </li>
            </ul>
            <ul class="navbar-nav ml-auto">
                <a class="btn btn-outline-danger my-2 my-sm-0" style="margin-right: 5px;" href="../logoff.php">Sair</a>
            </ul>
        </div>
    </nav>
    <div class="container" align="center" style="margin-top: 8rem;">
        <div class="row" style="width: 30rem;">
            <div class="card">
                <div class="card-body" align="left">
                        <form action="../control/controle.php?acao=cadastrar" method="POST" name="form">
                            <label>Nome do solicitante: </label>
                            <input type="text" name="nSolicitante">
                            <br>
                            <label>Contato: </label>
                            <input type="text" name="contatoSoli">
                            <br>
                            <label>Sobre a ordem de serviço: </label>
                            <input type="text" name="sobre">
                            <br>
                            <label>Trouxe equipamento?</label>
                            <input type="radio" name="trouxeEquip" value="Sim">Sim
                            <input type="radio" name="trouxeEquip" value="Não">Não
                            <br>
                            <label>Nome do executor: </label>
                            <input type="text" name="nExecutor">
                            <br>
                            <br><br>
                            <div align="center">
                                <input class="btn btn-info" type="submit" value="Cadastrar" onclick="return validarCampos()">
                                <input class="btn btn-warning" type="reset" name="limpar" value="Limpar campos">
                            </div>
                        </form>
                </div>
            </div>
    ...
    <script>
        function validarCampos(){
            var sol = form.nSolicitante;
            var contato = form.contatoSoli;
            var sobre = form.sobre;
            var tE = form.trouxeEquip;
            var exe = form.nExecutor;
            
            var selecionado = false;

            if(sol.value == "" || contato.value == "" || sobre.value == "" || tE.value == "" || exe.value == ""){
                alert("Preencha todos os campos!");
                nome.focus();
                return false;
            }

            for (var i = 0; i<comorbidade.length; i++){
                if(tE[i].checked){
                    selecionado = true;
                    break;
                }
            ...
            if(!selecionado){
                alert("Vc não respondeu se possui comorbidade!");
                return false;
            }
            return true;
        }
